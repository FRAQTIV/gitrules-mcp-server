class GitrulesMcp < Formula
  ...
  def install
    ...
    libexec.install Dir["*"]
    (bin/"mcp-git-rules").write_env_script libexec/"dist/index.js", NODE_PATH: "#{libexec}/node_modules", NODE_OPTIONS: "--experimental-modules"
  end

  test do
    output = shell_output("#{bin}/mcp-git-rules --transport=stdio 2>/dev/null | head -n1", 0)
    assert_match "api_version", output
  end
end
